...
% psi = 2*(0.9*dirac(phi)*dirac(mue)+0.1/4/pi*(dirac(mue+1)+dirac(mue-1)))
    ...
    case 'FullMoment'
        ...
        for i=0:parameter.Method.HighestMoment
            for j=0:i
                ...
                if i==0
                    psiMom = @(x,y) 1.*(x.^2+y.^2<=0.25)+(x.^2+y.^2>0.25).*1/2*beta(1/2,1+1/2*(k1+k2))*beta((k1+1)/2,(k2+1)/2)*(1+(-1)^k1+(-1)^(k1+k2)+(-1)^k2)*psi(x,y);
                    for k=1:6
                        M(:,k) = (bsxfun(@times,psiMom(QuadraturePointsx,QuadraturePointsy),PVal(k,:)))*Weights/dx/dy;
                    end
                else
                    if j==0
                        psiMom = @(x,y) (x.^2+y.^2<=0.25).*0.9+(x.^2+y.^2>0.25).*1/2*beta(1/2,1+1/2*(k1+k2))*beta((k1+1)/2,(k2+1)/2)*(1+(-1)^k1+(-1)^(k1+k2)+(-1)^k2)*psi(x,y);
                        for k=1:6
                            M(:,k) = (bsxfun(@times,psiMom(QuadraturePointsx,QuadraturePointsy),PVal(k,:)))*Weights/dx/dy;
                        end
                    else
                        psiMom = @(x,y) (x.^2+y.^2<=0.25).*0+(x.^2+y.^2>0.25).*1/2*beta(1/2,1+1/2*(k1+k2))*beta((k1+1)/2,(k2+1)/2)*(1+(-1)^k1+(-1)^(k1+k2)+(-1)^k2)*psi(x,y);
                        for k=1:6
                            M(:,k) = (bsxfun(@times,psiMom(QuadraturePointsx,QuadraturePointsy),PVal(k,:)))*Weights/dx/dy;
                        end
                    end
