...
psi = @(x,y) 1/4/pi*max(1e-4,exp(-10*((x-x0).^2/(sigma_x^2)+(y-y0).^2/(sigma_y^2))));
...
switch parameter.Method.Model
    ...
    case 'MQM'
        ...
        for i=1:NumberMoments
            psiMom = @(x,y) A(i)*psi(x,y);
            for k=1:6
                    Y(:,k,i) = (bsxfun(@times,psiMom(QuadraturePointsx,QuadraturePointsy),PVal(k,:)))*Weights/dx/dy;
            end
        end
    ...
    case 'SphericalHarmonics'
        ...
        Y = zeros(size(x,1),parameter.Solving.NumberPolynomials,NumberMoments);
        for i=1:NumberMoments
            psiMom = @(x,y) parameter.OptimPara.phiIso(i)*psi(x,y);
            for k=1:6
                Y(:,k,i) = (bsxfun(@times,psiMom(QuadraturePointsx,QuadraturePointsy),PVal(k,:)))*Weights/dx/dy;
            end
        end
